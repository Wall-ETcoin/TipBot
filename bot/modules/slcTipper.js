...
let walletConfig = config.get('slc').config;
let paytxfee = config.get('slc').paytxfee;
const slc = new bitcoin.Client(walletConfig);

exports.commands = ['tipslc'];
exports.tipslc = {
    usage: '<subcommand>',
    description:
    ...
    process: async function(bot, msg, suffix) {
        ...
        switch (subcommand) {
            ...
            case 'deposit':
                ...
                break;
            case 'map':
                privateorSpamChannel(msg, channelwarning, doMap, [tipper]);
                break;
            case 'withdraw':
...
function doMap(message, tipper) {
    getMNsIP(tipper, function(err, address) {
        if (err) {
            message.reply('Error getting your SuperLumic coin (SLC) deposit address.'+' Error: ' + err).then(message => message.delete(10000));
        } else {
            message.channel.send({ embed: {
    ...
    slc.getBalance(tipper, 1, function(err, balance) {
        if (err) {
            ...
            if (message.mentions.users.first().id) {
                sendSLC(bot, message, tipper, message.mentions.users.first().id.replace('!', ''), amount, prv);
            } else {
...
function sendSLC(bot, message, tipper, recipient, amount, privacyFlag) {
    getAddress(recipient.toString(), function(err, address) {
        if (err) {
            message.reply(err.message).then(message => message.delete(10000));
        } else {
            slc.sendFrom(tipper, address, Number(amount), 1, null, null, function(err, txId) {
                if (err) {
                    message.reply(err.message).then(message => message.delete(10000));
                } else {
                    if (privacyFlag) {
                        ...
                        if (
                                message.content.startsWith('!tipslc private ')
                           ) {
...
function getMNsIP(userId, cb) {
    var batch = [];
    batch.push({
        method: 'masternodelist',
        params: ['addr']
    });
    slc.cmd(batch, function(err, balance, resHeaders){
        ...
        if (err) {
            ...
            /*slc.getNewAddress(userId, function(err, address) {
              if (err) {
              cb(err);
              } else {
              cb(null, address);
        ...
        //  cb(null, balance); //console.log('Balance:', balance);
    });
}

function getAddress(userId, cb) {
    slc.getAddressesByAccount(userId, function(err, addresses) {
        if (err) {
            cb(err);
...
function getValidatedAmount(amount) {
    amount = amount.trim();
    if (amount.toLowerCase().endsWith('slc')) {
...
function txLink(txId) {
    return 'http://explorer.superlumic.co/tx/' + txId;
}

function addyLink(address) {
    return 'http://explorer.superlumic.co/address/' + address;
}
